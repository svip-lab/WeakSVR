...
class VerificationDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.storage_data_path = r'/storage/data/dongsx/dataset/CSV/frames/'
        logger.info(
    ...
    def __getitem__(self, index):
        ...
        sample = {
            # 'index': index,
            'Data': data_path,
            'label': int(data_path_split[0][-1:])-1,
            'clips1': self.sample_clips(self.storage_data_path + data_path_split[1]),
            'clips2': self.sample_clips(self.storage_data_path + data_path_split[2]),
            'clips3': self.sample_clips(self.storage_data_path + data_path_split[3]),
            'clips4': self.sample_clips(self.storage_data_path + data_path_split[4]),
            'clips5': self.sample_clips(self.storage_data_path + data_path_split[5]),
            'label_token1': self._load_label_token(data_path_split[0]),
            # 'label_token_phrase1': self._load_label_token_phrase(data_path_split[0]),
        }
        return sample
    ...
    def _load_label_token_phrase(self, label_number):
        if self.mode != 'train':
            return -1
        js_path = self.txt_path.replace('train_pairs.txt', 'label_bank.json')
    ...
    def _load_label_token(self, label_number):
        ...
        js_path = self.txt_path.replace('text_retrieval.txt', 'label_bank.json')
    ...
    def sample_clips(self, video_dir_path):
        ...
        sampled_clips = []
        num_sampled_per_segment = 1 if self.mode == 'train' else 1
        ...
        for i in range(num_sampled_per_segment):
            ...
            for j in range(self.num_clip):
                ...
                if self.mode == 'train':
                    ...
                    frame_index = segments[j] + int((segments[j + 1] - segments[j]) / 4) * (i + 3)
...
def load_dataset_retrival_ddp(cfg, args, drop_last=True):
    ...
    if not args.pair and cfg.DATASET.MODE != 'test' and cfg.DATASET.MODE != 'val':
        txt_path = cfg.DATASET.TXT_PATH.replace('train_pairs.txt', 'train.txt')
...
if __name__ == "__main__":
    ...
    cfg.merge_from_file('/public/home/dongsx/svip/configs/train_config.yml')
    test_cfg.merge_from_file('/public/home/dongsx/svip/configs/eval_retrival_config.yml')

    dataset = VerificationDataset(mode=cfg.DATASET.MODE,
                                  dataset_name=cfg.DATASET.NAME,
                                  txt_path=test_cfg.DATASET.TXT_PATH,
                                  normalization=ImageNet_normalization,
                                  num_clip=cfg.DATASET.NUM_CLIP,
                                  augment=cfg.DATASET.AUGMENT,
                                  pair_data=True,
